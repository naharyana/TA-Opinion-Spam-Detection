...
shingling_spam = pd.read_csv('Data/shingling_spam_pf.csv', sep=',', encoding='latin-1')

#review_data = pd. read_csv('Data/......csv'), sep=',', encoding = 'latin-1'

#shingling_spam=shingling_spam.drop(['Unnamed: 0'], axis = 1)

shingling_spam_features = pd.DataFrame(columns = ['F1','F2','F3','F4','F5','F6','F7','F8','F9'
                                   ,'F10','F11','F12','F13','F14','F15','F16','F17'
                                   ,'F18','F19','F20','F21'])

shingling_spam = shingling_spam.rename(columns = {'Title_x':'Title_Review','Title_y':'Title_Product'})
...
#F1 : Jumlah feedback 
shingling_spam_features['F1'] = shingling_spam['Feedbacks']

#F2 : Jumlah helpful feedback
shingling_spam_features['F2'] = shingling_spam['HelpfulFeedbacks']

#F3 : Persentasi helpful feedback 
shingling_spam_features['F3'] = shingling_spam['HelpfulFeedbacks']/shingling_spam['Feedbacks']

#F4 : Panjang judul review 
shingling_spam_features['F4'] = shingling_spam['Title_Review'].apply(len)

#F5 : Panjang body review
shingling_spam['Body'].fillna('none',inplace=True)  
shingling_spam_features['F5'] = shingling_spam['Body'].apply(len)
...
m=0        
for reviewid in shingling_spam.loc[:,'ReviewID']:
    for j in range (len(f6)): 
        if f6['ReviewID'][j]==reviewid: 
            shingling_spam_features['F6'][m]=F6[j]
    m+=1
...
for reviewid in shingling_spam.loc[:,'ReviewID']:
    for j in range (len(f7)): 
        if f7['ReviewID'][j]==reviewid: 
            shingling_spam_features['F7'][n]=F7[j]
    n+=1
    

#F8 : Biner apakah sebuah review itu adalah review pertama atau tidak 
j=0
for order in shingling_spam.loc[:,'ReviewOrder']:
    if (order == 1):
        shingling_spam_features['F8'][j]= 1
    else:
        shingling_spam_features['F8'][j] = 0
    j+=1
...
for reviewid in shingling_spam.loc[:,'ReviewID']:
    for j in range (len(f9)): 
        if f9['ReviewID'][j]==reviewid: 
            shingling_spam_features['F9'][n]=F9[j]
    n+=1
...
y=len(shingling_spam)
a=[0 for data in range(y)]
k=0
for review in shingling_spam.loc[:,'Body']:
    #if (k==1):
...
shingling_spam_features['F11'] = b

shingling_spam.to_csv('shingling_spam.csv',index=False)
...
for review in shingling_spam.loc[:,'Body']:
    for j in range (y):
        if shingling_spam['Brand'][j] in review:
            c[k]+=1
            #print (shingling_spam['Brand'][j] in i)
            #print (shingling_spam['Brand'][j])    
    k+=1

shingling_spam_features['F13'] = c
...
shingling_spam_features['F14'] = d/shingling_spam_features['F5']
...
shingling_spam_features['F15'] = e/shingling_spam_features['F5']
...
shingling_spam_features['F16'] = f/shingling_spam_features['F5']

#F17 : Rating review.  
shingling_spam_features['F17'] = shingling_spam['Rating']
...
n=0            
for prodid in shingling_spam.loc[:,'ProductID']:
    for j in range (len(pr)): 
        if pr[j]==prodid:
            shingling_spam_features['F18'][n]=md[j]
    n+=1

#F19 : Kategori apakah review itu adalah bagus (rating ≥ 4), jelek (rating ≤ 2,5) atau standar (rating < 4𝑑𝑎𝑛 > 2,5)
j=0
for i in shingling_spam.loc[:,'Rating']:
    if i>=4 :
        shingling_spam_features['F19'][j]=2
    elif (i<4&i>2.5):
        shingling_spam_features['F19'][j]=1
    elif i<=2.5:
        shingling_spam_features['F19'][j]=0
    j+=1
...
for reviewid in shingling_spam.loc[:,'ReviewID']:
    for j in range (len(f9)): 
        if f9['ReviewID'][j]==reviewid: 
            shingling_spam_features['F20'][n]=F20[j]
    n+=1
...
for reviewid in shingling_spam.loc[:,'ReviewID']:
    for j in range (len(f9)): 
        if f9['ReviewID'][j]==reviewid: 
            shingling_spam_features['F21'][n]=F21[j]
    n+=1
...
shingling_spam_features.to_csv('testfeatures.csv', index=False)
shingling_spam_features.to_excel('testfeatures.xlsx', index=False)
...
for j in range(y):
    h=[]
    h.append(shingling_spam['Body'][j])   
    for i in shingling_spam.loc[j, : ]:
        i=str(i)
...
shingling_spam_features['F12'] = mean_cosine

shingling_spam_features.to_csv('shinglingspamfeatures.csv', index=False)
shingling_spam_features.to_excel('shinglingspamfeatures.xlsx', index=False)
